...
const PropertyOnboardingModal = ({ isOpen, onClose, propertyData }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/api/owners/${ownerId}/onboarding`, onboardingData);
      
      onClose();
    } catch (error) {
      console.error('Failed to save onboarding data:', error);
      alert('Det oppsto en feil ved lagring av data. Vennligst prøv igjen.');
    } finally {
  ...
  return (
    <div>
      ...
      <div className="space-y-4 mb-8">
        <StrategyCard
          selected={formData.rentalStrategy === 'short'}
          onClick={() => updateFormData('rentalStrategy', 'short')}
          icon={
          ...
          description="Airbnb, Booking.com. Høyest inntekt."
        />
        <StrategyCard
...
const Step3 = ({ formData, addRoom, removeRoom, addFurniture, removeFurniture, updateFurniture, toggleAmenity }) => {
  useEffect(() => {
    // Initialize with default rooms on mount
    if (formData.livingRooms.length === 0) {
      addRoom('living');
    }
    if (formData.bedrooms.length === 0) {
      addRoom('bedroom');
    }
    if (formData.bathrooms.length === 0) {
      addRoom('bathroom');
    }
  }, []);
...
const StrategyCard = ({ selected, onClick, icon, title, description, badge, isPurple }) => (
  <div
    onClick={onClick}
  ...
  >
    ...
    <div>
      <div className={`font-semibold text-lg ${isPurple && 'text-purple-600'}`}>{title}</div>
...
const PhotoOption = ({ selected, onClick, iconBg, iconColor, icon, title, description, price }) => (
  <div
    onClick={onClick}
    className={`border rounded-xl p-5 cursor-pointer transition-all flex items-center gap-5 ${
      selected ? 'border-2 border-gray-900' : 'border border-gray-300 hover:border-gray-900'
    }`}
  >
